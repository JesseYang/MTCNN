...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        with argscope(Conv2D, kernel_shape=3, nl = tf.identity, stride=1, padding="VALID"):
            ...
            classification_conv = (LinearWrap(p_net_conv)
                .Conv2D("conv4_1", 2, kernel_shape=1, nl = tf.identity)())
            regression_conv = (LinearWrap(p_net_conv)
                .Conv2D("conv4_2", 4, kernel_shape=1, nl = tf.identity)())
        ...
        # top_loss = tf.nn.top_k(classification_loss, k=round(0.7 * classification_loss.shape[0]))
