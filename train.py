...
class Model(ModelDesc):
    ...
    def _get_inputs(self):
        return [InputDesc(tf.uint8, [None, self.img_size, self.img_size, 3], 'input'),
                ...
                InputDesc(tf.float32, [None, 4], 'landmark')]
    def _build_graph(self, inputs):
        image, label, landmark = inputs
        # pdb.set_trace()
        ...
        if self.net_format == 'P-Net':
            ...
            classification_image = self._p_net_conv(classification_image, cfg.channels_12, cfg.kernel_size_12, 2, 1)
            ...
            regression_image = self._p_net_conv(regression_image, cfg.channels_12, cfg.kernel_size_12, 4, 1)
    ...
    def _p_net_conv(self, object, channels, kernel_size, out_channel, out_kernel_size):
        with tf.variable_scope("face_classification") as scope:
            for layer_idx, conv in enumerate(channels):
                # layer_input = tf.identity(object)
                # pdb.set_trace()
                object = Conv2D('conv.{}'.format(layer_idx),
                    object,
                    channels[layer_idx],
                    (kernel_size[layer_idx],kernel_size[layer_idx]),
                    'VALID')
                object = tf.nn.relu(object)
                if layer_idx == 0:
                    object = tf.nn.max_pool(object,[1, kernel_size[layer_idx], kernel_size[layer_idx], 1],[1, 2, 2, 1], 'SAME')

        return Conv2D('conv.{}'.format(layer_idx + 1),object,out_channel,(out_kernel_size,out_kernel_size),'VALID')

    def _r_net_conv(self, object, channels, kernel_size):
        with tf.variable_scope("face_classification") as scope:
            for layer_idx, conv in enumerate(channels):
                # layer_input = tf.identity(object)
                pdb.set_trace()
                ...
                object = tf.nn.relu(object)
                if layer_idx != 2:
                    object = tf.nn.max_pool(object,[1, 3, 3, 1],[1, 2, 2, 1], 'SAME')
        # pdb.set_trace()
        object = tf.reshape(object, [-1, 3*3*64])
    ...
    def _o_net_conv(self, object, channels, kernel_size):
        with tf.variable_scope("face_classification") as scope:
            for layer_idx, conv in enumerate(channels):
                # layer_input = tf.identity(object)
                # pdb.set_trace()
                ...
                if layer_idx < 2:
                    ...
                    object = tf.nn.max_pool(object,[1, 2, 2, 1],[1, 2, 2, 1], 'SAME')
        pdb.set_trace()
        object = tf.reshape(object, [-1, 3*3*64])
        w = tf.truncated_normal([3*3*64, 128], stddev = 0.1)
        b = tf.constant(0.1, shape = [128], stddev = 0.1)
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            ScheduledHyperParamSetter('learning_rate',
                                      ...
                                      #new learning_rate
                                      [(0, 1e-3), (5, 2e-3), (15, 3e-3), (30, 2e-4), (50, 1e-4)]),
