...
class Model(ModelDesc):
    ...
    def _get_inputs(self):
        return [InputDesc(tf.float32, [None, cfg.img_size_24, cfg.img_size_24, 3], 'input'),
    ...
    def _build_graph(self, inputs):
       ...
       with argscope(Conv2D, kernel_shape=3, nl=tf.identity, stride=1, padding="VALID"):
           ...
           .Conv2D("conv3", 64, kernel_shape=2)
           .PReLU(name="relu3")
           .FullyConnected('fc0', out_dim=128)
           .PReLU(name="relu4")())
...
def get_config(args):
    ...
    return TrainConfig(
        ...
        callbacks=[
           ...
           #                            # new learning_rate
                                     [(0, 1e-5)]),
            # PeriodicTrigger(InferenceRunner()),
        ...
        # steps_per_epoch=10,
        # steps_per_epoch=2000000 / int(args.batch_size),####each epoch run how much num iterators
        # max_epoch=29,
        max_epoch=1000000,
    )
